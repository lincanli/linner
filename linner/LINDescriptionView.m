...
@interface LINDescriptionView ()
@property (strong, nonatomic) UIScrollView* baseScrollView;
@property (strong, nonatomic) UIView* baseEditorView;
@property (strong, nonatomic) UITextView* descriptionEditor;
@end

@implementation LINDescriptionView
...
- (void)viewDidLoad {
    ...
    self.baseEditorView = [[UIView alloc]initWithFrame:CGRectMake(0, 10, self.view.frame.size.width, 44*3)];
    self.baseEditorView.backgroundColor = [UIColor whiteColor];
    self.descriptionEditor = [[UITextView alloc]initWithFrame:CGRectMake(10, 2, self.baseEditorView.frame.size.width-20, self.baseEditorView.frame.size.height)];
    self.descriptionEditor.editable = YES;
    [self.baseEditorView addSubview:self.descriptionEditor];
...
-(void) confirmButton: (UIBarButtonItem *)sender
{
    ...
    self.userObject.userDescription = self.descriptionEditor.text;
    [dataModel save:&error];
    
    LINSettingDataHelper* helper = [LINSettingDataHelper alloc];
    [helper saveForUserInfo:self.userObject.userDescription forKey:@"userDescription"];
